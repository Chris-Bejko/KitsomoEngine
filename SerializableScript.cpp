...
{
    for (auto& [name, entry] : vectorFields)    entry.serialize();
    for (auto& [name, entry] : vectorPtrFields) entry.serialize();
    for (auto& [name, field] : entityPtrFields)
        if (*field.ptr) field.guidStorage = (*field.ptr)->GetGUID();
...
void SerializableScript::ResolvePointers()
{
    for (auto& [name, field] : entityPtrFields)
    {
        if (field.guidStorage.empty()) continue;
        for (auto& e : Engine::get().GetManager()->GetEntities())
            if (e->GetGUID() == field.guidStorage) { *field.ptr = e.get(); break; }
    }
    ...
    for (auto& [name, field] : componentPtrFields)
    {
        ...
        for (auto& e : Engine::get().GetManager()->GetEntities())
        {
            if (e->GetGUID() != entityGUID) continue;
            for (auto& comp : e->GetComponents())
                if (comp->GetGUID() == compGUID) { *field.ptr = comp.get(); break; }
        }
    }
