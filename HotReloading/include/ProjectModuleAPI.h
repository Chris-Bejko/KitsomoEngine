...
template <typename T>
void RegisterProjectComponent(RegisterProjectComponentFn registerFn, const char *name)
{
    ComponentID id = ComponentTypeRegistry::get().GetOrCreateID(name);

    ComponentTypeRegistry::get().AssociateType<T>(id);

    registerFn(name, ComponentRegistrationMeta<T>::allowMultiple, &ProjectModuleAddDefault<T>, &ProjectModuleApplySerialized<T>);
}