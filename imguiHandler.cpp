...
void ImguiHandler::Update(sf::Time rest)
{
    ...
    if (savePressed) DrawSaveDialog();
    if (loadPressed) DrawLoadDialog();
...
void ImguiHandler::DrawToolbar()
{
    ...
    case EngineState::Running:
        if (stateButton("  Play", COLOR_SUCCESS, pressed))  OnPlay();
        ImGui::SameLine();
        if (stateButton("  Save", COLOR_ACCENT, pressed))   OnSave();
        ImGui::SameLine();
        if (stateButton("  Load", COLOR_ACCENT, pressed))   OnLoad();
        break;

    case EngineState::PlayMode:
        if (stateButton(" Pause", COLOR_WARNING, pressed))  OnPause();
        ImGui::SameLine();
        if (stateButton("  Save", COLOR_ACCENT, pressed))   OnSave();
        ImGui::SameLine();
        if (stateButton("  Load", COLOR_ACCENT, pressed))   OnLoad();
    ...
    if (ImGui::Button(" Reset", ImVec2(70, 28))) OnReset();
    ImGui::PopStyleColor(2);
...
{
    if (Engine::get().GetCurrentState() != EngineState::Running) return;
...
void ImguiHandler::OnPlay()  { savePressed = false; loadPressed = false; Engine::get().SetEngineState(EngineState::PlayMode); }
void ImguiHandler::OnPause() { Engine::get().SetEngineState(EngineState::Paused); }
void ImguiHandler::OnReset() { savePressed = false; loadPressed = false; Engine::get().SetEngineState(EngineState::Running); }
void ImguiHandler::OnSave()  { str = "saveFile.txt"; str.resize(255, '\0'); savePressed = true; }
void ImguiHandler::OnLoad()  { str = "saveFile.txt"; str.resize(255, '\0'); loadPressed = true; }
